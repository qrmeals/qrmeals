  ...
  <div class="container">
    <a class="navbar-brand m-0" >
      @if(!config('settings.hide_project_branding'))
        <!-- Restaruant Branding -->
        <img src="{{ $vendor->logowidedark }}" class="navbar-brand-img h-80 w-30" alt="...">
      @else
        <!-- Admin branding -->
        <img src="{{ config('global.site_logo_dark') }}" class="navbar-brand-img h-80 w-30" alt="...">
      @endif
    </a>
    ...
    <div class="col-auto">
      <button onclick="showOrders()" type="button" class="btn bg-gradient-primary my-1 me-1">
        ...
        <span class="badge badge-md badge-circle badge-floating badge-primary border-white" id="ordersCount">{{ $activeOrdersCount }}</span>
      </button>
